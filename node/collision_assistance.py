...
class CollisionAssistance:
    ...
    def lidar_callback(self, data):
        ...
        vel_d=(1-self.etha_vel)*self.vel_x+self.etha_vel*self.vel_joy
        
        if self.vel_joy >=0 and vel_d < 0:
            vel_d = 0
        
        u0=self.etha_delta*math.tan(self.steer_joy)+(1-self.etha_delta)*f_tot_y*self.wheelbase/max(abs(self.vel),0.001)**2
