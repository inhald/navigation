...
class GapBarrier:
    ...
    def lidar_callback(self, data):    
        ...
        if self.drive_state == "normal":
            ...
            # rospy.loginfo(str(obstacle_points_r))
            ...
            if self.vel >= 0.01 or self.vel <= -0.01:
                ...
                delta_d = math.atan((self.wheelbase*(self.k_p*d_tilde+self.k_d*d_tilde_dot))/((self.vel*self.vel)*(-wl_h[1]+wr_h[1])))
        
            else:
                delta_d = 0


            # rospy.loginfo(str(delta_d))
            ...
            if velocity <= self.velocity_zero:
                ...
                if self.stopped_time >= self.stop_time1:
                    ...
                    rospy.loginfo("turn_angle: " + str(self.turn_angle))
                
                # rospy.loginfo(str(self.turn_angle))
